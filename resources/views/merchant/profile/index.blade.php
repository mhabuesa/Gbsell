      ...
      <div class="bg-primary-dark-op">
        <div class="content content-full text-center">
          <div class="my-3">
            ...
            @else
                <img class="img-avatar img-avatar-thumb" src="{{asset(Auth::guard('merchant')->user()->photo)}}" alt="">
            @endif
          </div>
    ...
    <div class="content content-boxed">
      <div class="block block-rounded">
        ...
        <div class="block-content">
          <form action="{{route('profile.update' , Auth::guard('merchant')->user()->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row push">
              <div class="col-lg-8 col-xl-5 m-auto">
                <div class="mb-4">
                  <label class="form-label" for="one-profile-edit-name">Name</label>
                  <input type="text" class="form-control" id="one-profile-edit-name" name="name" placeholder="Enter your name.." value="{{Auth::guard('merchant')->user()->name}}">
                  @error('name')
                ...
                <div class="mb-4">
                  ...
                  <input type="phone" class="form-control" id="phone" name="phone" placeholder="Enter your phone.." value="{{Auth::guard('merchant')->user()->phone}}">
                  @error('phone')
                    <small class="text-danger">{{ $message }}</small>
                  @enderror
                </div>
                <div class="mb-4">
                  ...
                  <input type="email" class="form-control" id="one-profile-edit-email" name="email" placeholder="Enter your email.." value="{{Auth::guard('merchant')->user()->email}}" disabled>
                  @error('email')
                    <small class="text-danger">{{ $message }}</small>
                  @enderror
                </div>
                <div class="mb-4">
                  ...
                  <div class="mb-4">
                    ...
                    @else
                        <img class="img-avatar" id="image" src="{{asset(Auth::guard('merchant')->user()->photo)}}" alt="">
                    @endif
                  </div>
                  <div class="mb-4">
                    <label for="image" class="form-label">Choose a new avatar</label>
                    <input class="form-control" id="image" type="file" accept="image/*" name="photo" onchange="document.getElementById('image').src = window.URL.createObjectURL(this.files[0])">
                    @error('photo')
                      <small class="text-danger">{{ $message }}</small>
                    @enderror
                  </div>
